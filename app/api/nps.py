...
def get_parks(activity):
    ...
    if session.get("park_data", '') == '':
        session["park_data"] = get_data('activities/parks', {'id': activity})['data'][0]['parks']
    number_of_parks = len(session["park_data"])
    end = 0
    ...
    parks_list = []
    for park in session["park_data"][start:end]:
    ...
    return render_template('listofparks.html', data = parks_list, url = url, hide = hide_next_menu)
